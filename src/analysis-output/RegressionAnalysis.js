      ...
      <div>
        <table style={{ width: "300px", marginRight: "20px" }}>
          <tbody>
            ...
            <tr>
              <td className="header-background large">p-value</td>
...
export default function RegressionAnalysis({ data, setPopup }) {
  ...
  useEffect(
    () => {
      setEquationTemplates({
        linearEquationTemplate: `${colY.label} = ${
          linearRegressionCoefficients[0].toFixed(6) / 1
        } ${addOrSubtract(
          linearRegressionCoefficients[1].toFixed(6) / 1,
        )} ${Math.abs(linearRegressionCoefficients[1].toFixed(6) / 1)} * ${
          colX.label
        }`,
        quadraticEquationTemplate: generateEquationTemplate(
          degree2PolyCoefficients,
          colX.label,
          colY.label,
        ),
        cubicEquationTemplate: generateEquationTemplate(
          degree3PolyCoefficients,
          colX.label,
          colY.label,
        ),
        quarticEquationTemplate: generateEquationTemplate(
          degree4PolyCoefficients,
          colX.label,
          colY.label,
        ),
        degree5EquationTemplate: generateEquationTemplate(
          degree5PolyCoefficients,
          colX.label,
          colY.label,
        ),
        degree6EquationTemplate: generateEquationTemplate(
          degree6PolyCoefficients,
          colX.label,
          colY.label,
        ),
        centeredQuadraticEquationTemplate: generateEquationTemplate(
          centered2PolyCoefficients,
          colX.label,
          colY.label,
        ),
        centeredCubicEquationTemplate: generateEquationTemplate(
          centered3PolyCoefficients,
          colX.label,
          colY.label,
        ),
        centeredQuarticEquationTemplate: generateEquationTemplate(
          centered4PolyCoefficients,
          colX.label,
          colY.label,
        ),
        centeredDegree5EquationTemplate: generateEquationTemplate(
          centered5PolyCoefficients,
          colX.label,
          colY.label,
        ),
        centeredDegree6EquationTemplate: generateEquationTemplate(
  ...
  const title = `Bivariate Fit of ${colY.label} ${
