...
export default function DistributionAnalysis({ data, setPopup }) {
	const { colObj, vals, skew, kurtosis, numberOfBins, sem, ci } = data;
	...
	return (
		<Popup key={data.id} id={data.id} title={title} windowWidth={500} setPopup={setPopup}>
			<div id="popupcontainer" style={{ textAlign: 'center' }}>
				...
				/>
				<div>Bins</div>
				<InputNumber min={1} max={50} defaultValue={bins} onChange={(val) => setBins(val)} />
				<SummaryStatistics
					sem={sem}
					ci={ci}
					boxDataSorted={boxDataSorted}
...
const SummaryStatistics = ({ sem, ci, boxDataSorted, vals, skew, kurtosis }) => (
