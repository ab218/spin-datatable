...
export default function ContingencyD3Chart({
  colX,
  colY,
  groups,
  data,
    ...
    () => {
      if (d3Container.current) {
        ...
        stackedGroups
          ...
          .attr("height", (d) => {
            return (
              ((height - margin.top - margin.bottom) * newY(d[1] - d[0])) / n -
              groupPadding
            );
          })
          .attr("width", 10)
          ...
          .on("mousemove", function (d) {
            const groupHtml = `
              <div>
                <div><span class="tooltip-margin-left-bold">${
                  d.key
                }</span><span></span></div>
        ...
        cell
          ...
          .on("mousemove", function (d) {
            ...
            const tileHtml = `
            <div>
              <div class="tooltip-margin-left-bold">${value} row${
              value === 1 ? "" : "s"
            }</div>
              <div><span class="tooltip-margin-left-bold">${
                colX.label
