...
export function createRandomLetterString() {
  ...
  return (
    ...
    Array(10)
      .fill(undefined)
      .map((_) => alphabet.charAt(Math.floor(Math.random() * alphabet.length)))
      .join("") +
    "_"
  );
}
...
export const updateFiltersOnCellUpdate = (
  filters,
  cellValue,
  column,
  rowID,
  ...
  const newFilters = filters.map((filter) => {
    if (column.type === TEXT && filter.stringFilters[column.id]) {
      return filter.stringFilters[column.id].includes(cellValue)
        ...
        : {
            ...filter,
            filteredRowIDs: filter.filteredRowIDs.filter((id) => id !== rowID),
          };
    } else {
      const found = filter.numberFilters.find(
        (numFil) => numFil.id === column.id,
      );
      if (found) {
        return cellValue < found.max && cellValue > found.min
          ...
          : {
              ...filter,
              filteredRowIDs: filter.filteredRowIDs.filter(
                (id) => id !== rowID,
              ),
            };
      } else {
...
export const getAllFilteredRows = (rows, filters) => {
  ...
  const filteredRowsByString = Object.keys(stringFilters).length
    ...
    : [];
  const intersection = returnIntersectionOrNonEmptyArray(
    filteredRowsByRange,
    filteredRowsByString,
  );
  return intersection;
};
...
function rowValueWithinTheseColumnRanges(row) {
  ...
  return columns.every(
    (column) =>
      row[column.id] >= (column.min || column.colMin) &&
      row[column.id] <= (column.max || column.colMax),
  );
}
...
export function findCyclicDependencies(definitions, identifier) {
  ...
  const internalSearch = function (currentIdentifier) {
    ...
    const found =
      definitions[currentIdentifier] &&
      definitions[currentIdentifier].some(internalSearch);
...
export function generateUniqueRowIDs(cellSelectionRanges, rows) {
  const range = (start, end) =>
    Array(end - start + 1)
      .fill()
      .map((_, i) => start + i);
  const selectedRows = cellSelectionRanges.map((row) =>
    range(row.top, row.bottom),
  );
  const flattenedRowIndexes = selectedRows.flat();
  const rowIDs = rows
    .map((row, i) => flattenedRowIndexes.includes(i) && row.id)
    .filter((x) => x);
  return rowIDs;
}
...
export function generateUniqueColumnIDs(cellSelectionRanges, columns) {
  const range = (start, end) =>
    Array(end - start)
      .fill()
      .map((_, i) => start + i);
  const selectedColumns = cellSelectionRanges.map((column) =>
    range(column.left, column.right + 1),
  );
  const flattenedColumnIndexes = selectedColumns.flat();
  const columnIDs = columns
    .map((column, i) => flattenedColumnIndexes.includes(i) && column.id)
    .filter((x) => x);
  return columnIDs;
}
...
export function filterExcludedRows(rows, excludedRows, column) {
  return rows
    .map((row) => {
      let cellValue;
...
export function updateRow(row, columnID, columns, dependencyMap) {
  ...
  const getDependentColumns = (column) => {
    return column
      ? [column].concat(
          (dependencyMap[column] || emptyArray).map(getDependentColumns),
        )
      : emptyArray;
  };
  ...
  try {
    const updatedRow = dependencyList.reduce((rowSoFar, columnID) => {
      ...
      const updatedFormula = swapIDsForValuesInRow(
        columnFormula,
        rowSoFar,
        columns,
      );
      ...
      if (updatedFormula === "REFERROR") {
        return {
          ...
          [columnID]: {
            error: "#REF",
            errorMessage: "Variable referenced in formula is invalid",
          },
        };
      }
      if (containsLetters(updatedFormula)) {
        return {
          ...
          [columnID]: {
            error: "#FORMERR",
            errorMessage: "There is a problem with the formula",
          },
        };
      }
      return {
