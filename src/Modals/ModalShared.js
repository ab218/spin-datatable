...
export function CaratButtons({
  ...
  const notAllowedType =
    selectedColumn &&
    notAllowed &&
    notAllowed.includes(selectedColumn.modelingType);
  return (
          ...
          <div style={{ display: "flex", ...radioButton }} key={column.id}>
            ...
            <RemoveColumnButton
              removeColumn={() =>
                performingAnalysis
                  ? null
                  : removeColumnFromList(removeData, column)
              }
            />
          </div>
        ))}
...
export function SelectColumn({
  ...
  return (
    <div>
      ...
      <Card
        bordered
        style={{ height: "200px", marginTop: 20, ...cardWithBorder }}
      >
            ...
            columns.map((column) => {
              return (
                <Button
                  ...
                  onClick={(e) => {
                    if (e.metaKey) {
                      return setSelectedColumn((prev) =>
                        prev.find((p) => p.id === column.id)
                          ? prev
                          : prev.concat(column),
                      );
                    } else {
                  ...
                  type={
                    selectedColumn.find((col) => col.id === column.id)
                      ? "primary"
                      : "default"
                  }
                  value={column}
