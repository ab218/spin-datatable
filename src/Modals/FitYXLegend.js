...
function lightUpQuadrants(yData, xData) {
  ...
  if (yDataArr.length !== 0 && xDataArr.length !== 0) {
    ...
    return [REGRESSION, ONEWAY, LOGISTIC, CONTINGENCY];
  }
}
...
export default function VariableLegend({ yColData, xColData }) {
  ...
  return (
    <table
      style={{ marginTop: 30, textAlign: "center", width: 200, height: 200 }}
    >
      <tbody>
        <tr>
          <td style={{ width: "10%", height: "40%" }}>
            <ContinuousIcon
              styleProps={{
                opacity: yData === NOMINAL || yData === ORDINAL ? 0.3 : 1,
              }}
            />
          </td>
          <VariableLegendQuadrant
        ...
        <tr>
          ...
          >
            <OrdinalIcon
              styleProps={{
                opacity: yData === NOMINAL || yData === CONTINUOUS ? 0.3 : 1,
              }}
            />
            <NominalIcon
              styleProps={{
                opacity: yData === ORDINAL || yData === CONTINUOUS ? 0.3 : 1,
              }}
            />
          </td>
          <VariableLegendQuadrant
        ...
        <tr style={{ opacity: 1 }}>
          ...
          <td style={{ width: "40%", height: "10%" }}>
            <ContinuousIcon
              styleProps={{
                opacity: xData === NOMINAL || xData === ORDINAL ? 0.3 : 1,
              }}
            />
          </td>
          <td style={{ width: "40%", height: "10%" }}>
            <OrdinalIcon
              styleProps={{
                opacity: xData === NOMINAL || xData === CONTINUOUS ? 0.3 : 1,
              }}
            />
            <NominalIcon
              styleProps={{
                opacity: xData === CONTINUOUS || xData === ORDINAL ? 0.3 : 1,
              }}
            />
          </td>
        </tr>
